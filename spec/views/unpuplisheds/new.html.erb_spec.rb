...
RSpec.describe "unpuplisheds/new", type: :view do
  before(:each) do
    assign(:unpuplished, Unpuplished.new(
      :key => "MyString",
      :author => "MyText",
      :title => "MyText",
      :journal => "MyText",
  ...
  it "renders new unpuplished form" do
    render

    assert_select "form[action=?][method=?]", unpuplisheds_path, "post" do
